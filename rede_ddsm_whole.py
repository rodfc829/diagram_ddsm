"""Rede construída com a arquitetura DiaGRAM modificada de modo a conter apenas as partes treinadas com as imagens inteiras"""
...
def make_feature_extraction_model():
    feat_ext_input_patches = keras.Input(shape=(None, None, 3))  # (256,256,1)
...
X_train_whole, mean1, std1 = normalize(X_train_whole)
X_test_whole, mean2, std2 = normalize(X_test_whole)
...
@tf.function
def train_step(whole, labels_whole):
    with feat_ext_tape, tf.GradientTape() as ext_class_tape:
...
def train(epochs):
    for epoch in range(epochs):
        ...
        for image_batch_w, labels_batch_w in datagen.flow(X_train_whole, y_train_whole, batch_size=BATCH_SIZE):
            train_step(image_batch_w, labels_batch_w)
            count += 1
            if count >= 6 * len(X_train_whole)
                break
